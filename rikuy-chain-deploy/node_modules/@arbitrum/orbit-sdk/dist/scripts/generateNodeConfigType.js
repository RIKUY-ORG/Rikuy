...
const { objectType } = ts_morph_1.Writers;
function getNitroNodeImageTag() {
    const defaultNitroNodeTag = 'v3.6.0-fc07dd2';
    const argv = process.argv.slice(2);
    if (argv.length < 2 || argv[0] !== '--nitro-node-tag') {
        console.log(`Using default nitro-node tag since none was provided. If you want to specify a tag, you can do "--nitro-node-tag v2.2.2-8f33fea".`);
...
function parseCliOptions(fileContents) {
    const types = {
        string: 'string',
        strings: 'string[]',
        stringArray: 'string[]',
    ...
    lines = lines.filter((line) => line.startsWith('--') && !line.includes('--help') && !line.includes('--dev'));
...
function createCliOptionsNestedObject(options) {
    const result = {};
...
function getTypeRecursively(value) {
...
function main() {
    ...
    (0, child_process_1.execSync)(`docker run --rm ${nitroNodeImage} --help > ${nitroNodeHelpOutputFile} 2>&1`);
    // read and parse the file
    const content = (0, fs_1.readFileSync)(nitroNodeHelpOutputFile, 'utf8');
    const cliOptions = parseCliOptions(content);
    const cliOptionsNestedObject = createCliOptionsNestedObject(cliOptions);
    // create the new source file
    const sourceFile = new ts_morph_1.Project().createSourceFile('./src/types/NodeConfig.generated.ts', '', {
        overwrite: true,
    });
    // append header
    ...
    (0, fs_1.rmSync)(nitroNodeHelpOutputFile);
}
main();
